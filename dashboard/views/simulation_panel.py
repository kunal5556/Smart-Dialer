...
def render(client: SmartDialerClient) -> None:
    ...
    with st.form("simulation_form"):
        ...
        mode = st.selectbox("Dialing mode", MODES)
        agents = st.number_input("Agents", min_value=1, max_value=200, value=10)
        borrowers = st.number_input("Borrowers", min_value=1, max_value=5000, value=300)
        duration = st.number_input(
            "Simulated seconds", min_value=10.0, max_value=3600.0, value=300.0
        )
        time_scale = st.number_input("Time scale", min_value=1.0, max_value=300.0, value=60.0)
    ...
    st.divider()
    with st.spinner("Loading simulation history"):
